...
```tsx
import { Modal } from '@almond-ui/core';
```
...
<Callout>Size classes include breakpoint prefixes. The Modal width will be
  automatically adjusted on smaller screens.</Callout>
...
import { Modal, Card, Button } from '@almond-ui/core';
...
```tsx
import { Button, Card, Modal, Text } from '@almond-ui/core';
import { useState } from 'react';
...
```tsx
import { Button, Card, Modal } from '@almond-ui/core';
import { useState } from 'react';
import * as React from 'react';
import { X } from '@phosphor-icons/react';

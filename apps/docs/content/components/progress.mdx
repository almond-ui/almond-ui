...
<Callout>The `striped` prop must be set to `true` in order to use the
  `animated` prop.</Callout>
...
<Callout>On `xs` and `sm` sizes the value will be placed on the top side of the
  progress bar instead.</Callout>
...
<Callout>Adding a `label` and the `showValue` props together will push the
  value at right side on xs and sm sizes.</Callout>
